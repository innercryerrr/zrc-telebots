...
Telebot.prototype.sendone = async function (tttchatid, text, opts = {}) {
    await this.$n.sendMessage(tttchatid, text, opts)
}

Telebot.prototype.sendClickable = async function (ttchatid, label, content) {
    // buliding...
}
...
Telebot.prototype.sendimg = async function (ttchatid, _pathOrFileId, opts = {}) {
    await this.$n.sendPhoto(ttchatid, _pathOrFileId, opts);
}
...
Telebot.prototype.feedback = async function (ttchatid, actype, text, delay) {
   const ddelay = delay || 3000;
   await this.$n.sendMessage(ttchatid, '  ⏳ ' + text);
   this.$n.sendChatAction(ttchatid, actype);
   await util.delayRandom({min: delay, max: delay}, '   ...sending feedback()')
}
...
Telebot.prototype.debug = async function (propmap, 
                                            file, line, 
                                                timeout = 1000, 
                                                    disabled) {
    ...
    if (this.$debug) {
        ...
        clog('\n\n')
        clog(c.cyan, '  🐞 Debugging at:', file + ':' + line, '\n')
        
        for (let key in propmap) {
            clog(`  → ${key}:`, propmap[key])
        }

        await util.delayRandom({ 
            min: timeout, 
                max: timeout }, 
                    '\n Waiting debugger show infos... \n');

        clog(c.yellow, '  🐞 Debugging end.')
        clog('\n')
    }
...
Telebot.prototype.pixInvoice = async function (ttchatid, invd) {
    clog('\n      ...sending invoice \n\n')
    await this.$n.sendPhoto({
        chat_id: ttchatid,
        photo: invd.qrimgurl, 
        caption: invd.caption
    })
}
